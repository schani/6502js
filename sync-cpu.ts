...
export class SyncCPU implements CPU {
    ...
    private async compareStates(): Promise<void> {
        ...
        if (s1.a !== s2.a)
            throw new Error(`CPU divergence: A ${s1.a} != ${s2.a}`);
        if (s1.x !== s2.x)
            throw new Error(`CPU divergence: X ${s1.x} != ${s2.x}`);
        if (s1.y !== s2.y)
            throw new Error(`CPU divergence: Y ${s1.y} != ${s2.y}`);
        if (s1.sp !== s2.sp)
            throw new Error(`CPU divergence: SP ${s1.sp} != ${s2.sp}`);
        if (s1.p !== s2.p)
            throw new Error(`CPU divergence: P ${s1.p} != ${s2.p}`);
        if (s1.pc !== s2.pc)
            throw new Error(`CPU divergence: PC ${s1.pc} != ${s2.pc}`);
    }
}
