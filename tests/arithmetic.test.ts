...
describe("Arithmetic operations", async () => {
  it("should perform ADC immediate instruction", async () => {
    ...
    expect(await cpu.getAccumulator()).toBe(0x79); // 0x42 + 0x37 = 0x79
  ...
  it("should handle ADC with carry in", async () => {
    ...
    expect(await cpu.getAccumulator()).toBe(0x7A); // 0x42 + 0x37 + 1 = 0x7A
    expect(cpu.getProgramCounter()).toBe(2);
    expect(cycles).toBe(2);
  });
  ...
  it("should handle ADC with carry out", async () => {
    ...
    expect(await cpu.getAccumulator()).toBe(0x60); // 0xD0 + 0x90 = 0x160, truncated to 0x60
  ...
  it("should handle ADC with overflow", async () => {
    ...
    expect(await cpu.getAccumulator()).toBe(0xA0);
  ...
  it("should perform SBC immediate instruction", async () => {
    ...
    expect(await cpu.getAccumulator()).toBe(0x22); // 0x42 - 0x20 = 0x22
  ...
  it("should handle SBC with borrow", async () => {
    ...
    expect(await cpu.getAccumulator()).toBe(0x21); // 0x42 - 0x20 - 1 = 0x21
    expect(cpu.getProgramCounter()).toBe(2);
    expect(cycles).toBe(2);
  });
  ...
  it("should handle SBC with borrow out", async () => {
    ...
    expect(await cpu.getAccumulator()).toBe(0xF0); // 0x20 - 0x30 = 0xF0 (with borrow)
  ...
  it("should perform CMP immediate instruction", async () => {
    ...
    expect(await cpu.getAccumulator()).toBe(0x42); // Accumulator should not change
