...
import { createCPU } from "./tests/utils";
...
describe("6502 disassembler", () => {
  ...
  it("should disassemble immediate instructions", () => {
    ...
    // LDA #$42
    cpu.loadByte(0x1000, 0xA9);
    cpu.loadByte(0x1001, 0x42);
    expect(disassemble(cpu, 0x1000)).toEqual(["LDA #$42", 2]);

    // LDX #$FF
    cpu.loadByte(0x1002, 0xA2);
    cpu.loadByte(0x1003, 0xFF);
    expect(disassemble(cpu, 0x1002)).toEqual(["LDX #$FF", 2]);

    // CPY #$00
    cpu.loadByte(0x1004, 0xC0);
    cpu.loadByte(0x1005, 0x00);
  ...
  it("should disassemble zero page instructions", () => {
    ...
    // LDA $42
    cpu.loadByte(0x1000, 0xA5);
    cpu.loadByte(0x1001, 0x42);
    expect(disassemble(cpu, 0x1000)).toEqual(["LDA $42", 2]);

    // STA $FF
    cpu.loadByte(0x1002, 0x85);
    cpu.loadByte(0x1003, 0xFF);
  ...
  it("should disassemble zero page,X and zero page,Y instructions", () => {
    ...
    // LDA $42,X
    cpu.loadByte(0x1000, 0xB5);
    cpu.loadByte(0x1001, 0x42);
    expect(disassemble(cpu, 0x1000)).toEqual(["LDA $42,X", 2]);

    // LDX $30,Y
    cpu.loadByte(0x1002, 0xB6);
    cpu.loadByte(0x1003, 0x30);
  ...
  it("should disassemble absolute instructions", () => {
    ...
    // JMP $1234
    cpu.loadByte(0x1000, 0x4C);
    cpu.loadByte(0x1001, 0x34);
    cpu.loadByte(0x1002, 0x12);
    expect(disassemble(cpu, 0x1000)).toEqual(["JMP $1234", 3]);

    // LDA $ABCD
    cpu.loadByte(0x1003, 0xAD);
    cpu.loadByte(0x1004, 0xCD);
    cpu.loadByte(0x1005, 0xAB);
  ...
  it("should disassemble indexed indirect instructions", () => {
    ...
    // LDA ($10,X)
    cpu.loadByte(0x1000, 0xA1);
    cpu.loadByte(0x1001, 0x10);
    expect(disassemble(cpu, 0x1000)).toEqual(["LDA ($10,X)", 2]);

    // STA ($20,X)
    cpu.loadByte(0x1002, 0x81);
    cpu.loadByte(0x1003, 0x20);
  ...
  it("should disassemble indirect indexed instructions", () => {
    ...
    // LDA ($10),Y
    cpu.loadByte(0x1000, 0xB1);
    cpu.loadByte(0x1001, 0x10);
    expect(disassemble(cpu, 0x1000)).toEqual(["LDA ($10),Y", 2]);

    // STA ($30),Y
    cpu.loadByte(0x1002, 0x91);
    cpu.loadByte(0x1003, 0x30);
  ...
  it("should disassemble relative branch instructions", () => {
    ...
    cpu.loadByte(0x1000, 0xF0);
    cpu.loadByte(0x1001, 0x05);
    expect(disassemble(cpu, 0x1000)).toEqual(["BEQ $1007", 2]);

    // BNE -10 (branch backward 10 bytes)
    cpu.loadByte(0x1002, 0xD0);
    cpu.loadByte(0x1003, 0xF6); // -10 in two's complement
  ...
  it("should disassemble multiple instructions in sequence", () => {
    ...
    cpu.loadByte(0x1000, 0xA9);
    cpu.loadByte(0x1001, 0x42);
    cpu.loadByte(0x1002, 0xAA);
    cpu.loadByte(0x1003, 0xE8);
    cpu.loadByte(0x1004, 0x86);
    cpu.loadByte(0x1005, 0x10);
  ...
  it("should disassemble accumulator mode instructions", () => {
    ...
    // ASL A
    cpu.loadByte(0x1000, 0x0A);
    expect(disassemble(cpu, 0x1000)).toEqual(["ASL A", 1]);
    
    // LSR A
    cpu.loadByte(0x1001, 0x4A);
